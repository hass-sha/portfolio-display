...
export function findUserAuth(username: string, cb: any) {
  Users.find({username}).limit(1).then((user) => {
    if (user.length < 1) {
...
function checkExistingUser(username: string, emailAddress: string) {
  return new Promise((resolve, reject) => {
    ...
    Users.find({email: emailAddress}).then((email) => {
      if (email.length >= 1) {
        emailExists = true;
      }
      emailChecked = true;
    ...
    checkExistingUser(username, email).then((response) => {
      ...
      if (!checkResponse.exists) {
        securePassword(password).then((securePass) => {
          const user = new Users({username, password: securePass, displayName, email});
          user.save().then(() => resolve())
            .catch((err) => reject({source: 'insertUser.checkExistingUser.securePassword.user', error: err}));
        }).catch((err) => {
          reject(err);
        });
      } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    generateSalt(iterations).then((saltObj) => {
      ...
      crypto.pbkdf2(password, salt, iterations, 512, 'sha512', (err, derivedKey) => {
        if (err) {
          reject({source: 'securePassword.crypto.pbkdf2', error: err});
        }
        resolve(new PasswordModel({salt, hash: derivedKey.toString(), iterations}));
      });
    }).catch((err) => {
      reject(err);
    });
...
function passwordHash(password: string, salt: string, iterations: number, keylen: number, digest: string) {
  return new Promise((resolve, reject) => {
    crypto.pbkdf2(password, salt, iterations, keylen, digest, (err, derivedKey) => {
      if (err) {
        // return error
        reject({source: 'passwordHash.crypto.pbkdf2', error: err});
      }
      resolve(derivedKey);
    });
  });
...
export function verifyPassword(savedPassword: Password, password: string): Promise<boolean> {
  return new Promise((resolve, reject) => {
    passwordHash(password, savedPassword.salt, savedPassword.iterations, 512, 'sha512').then((newHash) => {
...
function generateSalt(length: number) {
  return new Promise((resolve, reject) => {
    crypto.randomBytes(Math.ceil(length / 2), (err, buffer) => {
      if (err) {
        reject({source: 'generateSalt', error: err});
      }
